<div>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('CSRD Compliance Dashboard') }}</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Corporate Sustainability Reporting Directive (EU) 2022/2464') }}</p>
            </div>
    ...
    @if($this->csrdApplicability['applicable'])
        <div class="mb-6 rounded-lg bg-green-50 dark:bg-green-900/20 p-4 border border-green-200 dark:border-green-800">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-green-800 dark:text-green-200">{{ __('CSRD Applicable') }}</h3>
                    <p class="mt-1 text-sm text-green-700 dark:text-green-300">
                        {{ __('Your organization is classified as') }} <strong>{{ ucfirst($this->csrdApplicability['category']) }}</strong>.
                        {{ __('First reporting year') }}: <strong>{{ $this->csrdApplicability['first_reporting_year'] }}</strong>
                    </p>
                </div>
            </div>
        </div>
    @else
        <div class="mb-6 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 p-4 border border-yellow-200 dark:border-yellow-800">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">{{ __('CSRD May Not Apply Yet') }}</h3>
                    <p class="mt-1 text-sm text-yellow-700 dark:text-yellow-300">
                        {{ __('Based on current thresholds, CSRD reporting may not be mandatory for your organization. However, voluntary adoption is recommended.') }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Overall Compliance') }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('ESRS 2 Disclosures') }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Transition Plan') }}</p>
                    ...
                    @else
                        <p class="mt-2 text-lg font-semibold text-red-600 dark:text-red-400">{{ __('Not Created') }}</p>
                    @endif
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('EU Taxonomy') }}</p>
                    @if($this->taxonomyReport)
                        <p class="mt-2 text-lg font-semibold text-gray-900 dark:text-white">
                            {{ number_format($this->taxonomyReport->turnover_aligned_percent ?? 0, 1) }}% {{ __('Aligned') }}
                        </p>
                    @else
                        <p class="mt-2 text-lg font-semibold text-red-600 dark:text-red-400">{{ __('Not Reported') }}</p>
                    @endif
                </div>
    ...
    @if($activeTab === 'overview')
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- ESRS 2 Progress -->
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">{{ __('ESRS 2 General Disclosures') }}</h3>
                <div class="space-y-4">
                    @foreach(['bp' => 'Basis for Preparation', 'gov' => 'Governance', 'sbm' => 'Strategy & Business Model', 'iro' => 'Impacts, Risks, Opportunities'] as $category => $label)
                        <div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                ...
                <div class="space-y-3">
                    <a href="{{ route('csrd.esrs2') }}" class="flex items-center p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div>
                            <p class="font-medium text-gray-900 dark:text-white">{{ __('ESRS 2 Disclosures') }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Manage general disclosures') }}</p>
                        </div>
                    </a>
                    <a href="{{ route('csrd.transition-plan') }}" class="flex items-center p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div>
                            <p class="font-medium text-gray-900 dark:text-white">{{ __('Climate Transition Plan') }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('ESRS E1-1 requirements') }}</p>
                        </div>
                    </a>
                    <a href="{{ route('csrd.taxonomy') }}" class="flex items-center p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div>
                            <p class="font-medium text-gray-900 dark:text-white">{{ __('EU Taxonomy Report') }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Article 8 disclosure') }}</p>
                        </div>
                    </a>
                    <a href="{{ route('csrd.due-diligence') }}" class="flex items-center p-3 rounded-lg border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <div>
                            <p class="font-medium text-gray-900 dark:text-white">{{ __('Value Chain Due Diligence') }}</p>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('LkSG / CSDDD compliance') }}</p>
                        </div>
                    </a>
                </div>
            </div>
    ...
    @elseif($activeTab === 'deadlines')
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('Upcoming CSRD Deadlines') }}</h3>
            </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="text-right">
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    {{ $deadline['days_remaining'] }} {{ __('days remaining') }}
                                </p>
                            </div>
                        </div>
                    </li>
    ...
    @elseif($activeTab === 'due-diligence')
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('Value Chain Due Diligence Status') }}</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('LkSG (German Supply Chain Act) / CSDDD compliance') }}</p>
                </div>
            ...
            @if($this->dueDiligence)
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="text-center p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Complaints Resolved') }}</p>
                    </div>
                </div>
            @endif
            ...
            <a href="{{ route('csrd.due-diligence') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                {{ $this->dueDiligence ? __('Manage Due Diligence') : __('Start Due Diligence Assessment') }}
