<div>
    ...
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
            {{ __('linscarbon.sites.import.description') }}
        </p>
    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm mb-6">
            <div class="flex items-center justify-between mb-4">
                <h4 class="font-medium text-gray-900 dark:text-white">
                    1. {{ __('linscarbon.sites.import.upload_file') }}
                </h4>
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        {{ __('linscarbon.sites.import.csv_file') }}
                    </label>
                ...
                <div class="flex items-center">
                    ...
                    <label for="hasHeader" class="ml-2 text-sm text-gray-700 dark:text-gray-300">
                        {{ __('linscarbon.sites.import.has_header') }}
                    </label>
                </div>
            </div>
        </div>
        ...
        @if($showMapping && count($previewData) > 0)
            <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm mb-6">
                <h4 class="font-medium text-gray-900 dark:text-white mb-4">
                    2. {{ __('linscarbon.sites.import.map_columns') }}
                </h4>

                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                    @foreach(['name' => __('linscarbon.sites.name'), 'code' => __('linscarbon.sites.code'), 'type' => __('linscarbon.sites.type'), 'address_line_1' => __('linscarbon.sites.address'), 'city' => __('linscarbon.sites.city'), 'postal_code' => __('linscarbon.sites.postal_code'), 'country' => __('linscarbon.sites.country'), 'floor_area_m2' => __('linscarbon.sites.floor_area'), 'employee_count' => __('linscarbon.sites.employees'), 'energy_rating' => __('linscarbon.sites.energy_rating'), 'construction_year' => __('linscarbon.sites.construction_year'), 'heating_type' => __('linscarbon.sites.heating_type')] as $field => $label)
                        <div>
                            ...
                            <select wire:change="setColumnMapping('{{ $field }}', $event.target.value)"
                                ...
                                <option value="">-- {{ __('linscarbon.sites.import.skip') }} --</option>
                                @foreach($previewData[0] ?? [] as $index => $value)
                                    <option value="{{ $index }}" {{ ($columnMapping[$field] ?? null) === $index ? 'selected' : '' }}>
                                        {{ __('linscarbon.sites.import.column') }} {{ $index + 1 }}: {{ Str::limit($value, 20) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                ...
                <div class="overflow-x-auto">
                    <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        {{ __('linscarbon.sites.import.preview') }} ({{ count($previewData) }} {{ __('linscarbon.sites.import.rows') }})
                    </h5>
                            ...
                            <tr>
                                @foreach($previewData[0] ?? [] as $index => $value)
                                    <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">
                                        {{ __('linscarbon.sites.import.col') }} {{ $index + 1 }}
                                    </th>
                                @endforeach
                            </tr>
                        </thead>
            ...
            <div class="flex items-center justify-end space-x-3">
                ...
                <button wire:click="import" type="button"
                    ...
                    <span wire:loading.remove wire:target="import">
                        {{ __('linscarbon.sites.import.import_button') }}
                    </span>
                    <span wire:loading wire:target="import" class="flex items-center">
                        ...
                        {{ __('linscarbon.sites.import.importing') }}
                    </span>
                </button>
            </div>
        @endif
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
            <div class="text-center mb-6">
                @if($importedCount > 0)
                    ...
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ __('linscarbon.sites.import.success_title') }}
                    </h4>
                @else
                    ...
                    <h4 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ __('linscarbon.sites.import.no_imports') }}
                    </h4>
                @endif
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div class="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-4 text-center">
                    ...
                    <p class="text-sm text-emerald-700 dark:text-emerald-300">{{ __('linscarbon.sites.import.imported') }}</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 text-center">
                    <p class="text-2xl font-bold text-gray-600 dark:text-gray-400">{{ $skippedCount }}</p>
                    <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('linscarbon.sites.import.skipped') }}</p>
                </div>
            </div>
            ...
            @if(count($importErrors) > 0)
                <div class="mb-6">
                    <h5 class="text-sm font-medium text-red-600 dark:text-red-400 mb-2">
                        {{ __('linscarbon.sites.import.errors') }} ({{ count($importErrors) }})
                    </h5>
                    <div class="max-h-40 overflow-y-auto">
                        <ul class="text-sm text-red-600 dark:text-red-400 space-y-1">
                            @foreach(array_slice($importErrors, 0, 10) as $error)
                                <li>{{ __('linscarbon.sites.import.row') }} {{ $error['row'] }}: {{ $error['message'] }}</li>
                            @endforeach
                            @if(count($importErrors) > 10)
                                <li class="text-gray-500">... {{ count($importErrors) - 10 }} {{ __('linscarbon.sites.import.more_errors') }}</li>
                            @endif
                        </ul>
                    </div>
                </div>
