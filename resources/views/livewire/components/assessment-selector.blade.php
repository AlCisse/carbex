<div x-data="{ open: false }" class="relative">
    <button @click="open = !open" type="button" class="flex items-center px-3 py-1.5 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
        ...
        @if($activeAssessment)
            ...
            @endif
        @else
            <span class="text-gray-400">{{ __('linscarbon.assessments.none') }}</span>
        @endif
    ...
    <div x-show="open"
        ...
        <div class="py-1">
            <div class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                {{ __('linscarbon.assessments.title') }}
            </div>
            ...
            @forelse($assessments as $assessment)
                <button wire:click="switchAssessment({{ $assessment->year }})"
                    ...
                    <span class="flex items-center">
                        {{ __('linscarbon.assessments.year_label', ['year' => $assessment->year]) }}
            ...
            @empty
                <div class="px-4 py-3 text-sm text-gray-500 text-center">
                    {{ __('linscarbon.assessments.empty_short') }}
                </div>
            @endforelse
