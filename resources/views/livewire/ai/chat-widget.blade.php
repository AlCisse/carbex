<div>
    ...
    <button
        ...
        aria-label="{{ __('linscarbon.ai.chat.assistant_name') }}"
    >
        @if($isOpen)
    ...
    >
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b" style="border-color: #e2e8f0; background: linear-gradient(135deg, #0d9488 0%, #0f766e 100%);">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-white">{{ __('linscarbon.ai.chat.assistant_name') }}</h3>
                    <p class="text-xs text-white/70">{{ __('linscarbon.ai.chat.subtitle') }}</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <button wire:click="startNewConversation" class="p-1.5 rounded-lg hover:bg-white/10 transition-colors" title="{{ __('linscarbon.ai.chat.new_conversation') }}">
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
            @if(empty($messages))
                ...
                <div class="text-center py-6">
                    ...
                    <h4 class="font-semibold mb-2" style="color: #0f172a;">{{ __('linscarbon.ai.chat.welcome') }}</h4>
                    <p class="text-sm mb-4" style="color: #64748b;">{{ __('linscarbon.ai.chat.welcome_description') }}</p>
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4 border-t bg-white" style="border-color: #e2e8f0;">
            @if($quota['enabled'] ?? false)
                ...
                <div class="flex items-center justify-between mt-2 text-xs" style="color: #94a3b8;">
                    <span>{{ __('linscarbon.ai.chat.powered_by') }}</span>
                    @if($quota['unlimited'] ?? false)
                        <span class="flex items-center gap-1">
                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>
                            {{ __('linscarbon.ai.chat.unlimited') }}
                        </span>
                    @else
                        <span class="flex items-center gap-1" title="{{ __('linscarbon.ai.chat.quota_daily') }}: {{ $quota['daily_used'] }}/{{ $quota['daily_limit'] }} | {{ __('linscarbon.ai.chat.quota_monthly') }}: {{ $quota['monthly_used'] }}/{{ $quota['monthly_limit'] }}">
                            @php
                            ...
                            <span class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded {{ $isLow ? 'bg-orange-100 text-orange-600' : 'bg-teal-50 text-teal-600' }}">
                                ...
                                {{ $remaining }} {{ $remaining > 1 ? __('linscarbon.ai.chat.remaining_plural') : __('linscarbon.ai.chat.remaining') }}
                            </span>
                        </span>
                    @endif
                </div>
            @else
                ...
                <div class="text-center py-3">
                    <p class="text-sm font-medium" style="color: #64748b;">{{ __('linscarbon.ai.chat.ai_not_available') }}</p>
                    <a href="{{ url('/settings/billing') }}" class="inline-flex items-center gap-1 mt-2 text-xs font-medium" style="color: #0d9488;">
                        {{ __('linscarbon.ai.chat.upgrade_premium') }}
