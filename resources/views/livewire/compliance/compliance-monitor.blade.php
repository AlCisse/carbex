    ...
    <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                {{ __('linscarbon.compliance.title') }}
            </h1>
            <p class="mt-1 text-gray-600 dark:text-gray-400">
                {{ __('linscarbon.compliance.subtitle') }}
            </p>
        </div>
        ...
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center space-x-2">
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ __('linscarbon.compliance.reporting_year') }}:
                </label>
            ...
            <button wire:click="openTaskModal"
                ...
                {{ __('linscarbon.compliance.add_task') }}
            </button>
        </div>
        ...
        <div class="mb-6 border-b border-gray-200 dark:border-gray-700">
            <nav class="flex space-x-8">
                @foreach(['overview' => __('linscarbon.compliance.tabs.overview'), 'csrd' => 'CSRD', 'iso' => 'ISO', 'tasks' => __('linscarbon.compliance.tabs.tasks')] as $tab => $label)
        ...
        @if($activeTab === 'overview')
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">CSRD {{ __('linscarbon.compliance.progress') }}</h3>
                        ...
                        <div class="mt-3 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <span>{{ __('linscarbon.compliance.disclosures') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">ISO {{ __('linscarbon.compliance.certifications') }}</h3>
                            <span class="text-2xl font-bold text-blue-600">{{ $this->isoStats['certified'] }}</span>
                        </div>
                        <div class="grid grid-cols-2 gap-2 text-xs">
                            <div class="flex items-center space-x-1">
                                <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                                <span class="text-gray-600 dark:text-gray-400">{{ __('linscarbon.compliance.certified') }}: {{ $this->isoStats['certified'] }}</span>
                            </div>
                            <div class="flex items-center space-x-1">
                                <span class="w-2 h-2 rounded-full bg-amber-500"></span>
                                <span class="text-gray-600 dark:text-gray-400">{{ __('linscarbon.compliance.in_progress') }}: {{ $this->isoStats['in_progress'] }}</span>
                            </div>
                        </div>
                        @if($this->isoStats['expiring_soon'] > 0)
                            <div class="mt-3 text-xs text-amber-600 dark:text-amber-400">
                                {{ $this->isoStats['expiring_soon'] }} {{ __('linscarbon.compliance.expiring_soon') }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('linscarbon.compliance.overdue_tasks') }}</h3>
                        ...
                        @else
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('linscarbon.compliance.no_overdue') }}</p>
                        @endif
                    </div>
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('linscarbon.compliance.upcoming') }}</h3>
                        ...
                        @else
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('linscarbon.compliance.no_upcoming') }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">CSRD {{ __('linscarbon.compliance.by_category') }}</h3>
                        ...
                        @foreach($this->csrdCategories as $key => $label)
                            ...
                            <div class="p-4 rounded-lg bg-{{ $color }}-50 dark:bg-{{ $color }}-900/20 border border-{{ $color }}-200 dark:border-{{ $color }}-800">
                                ...
                                <p class="mt-2 text-xs text-{{ $color }}-600 dark:text-{{ $color }}-400">
                                    {{ $categoryCompliant }} / {{ $categoryTotal }} {{ __('linscarbon.compliance.requirements') }}
                                </p>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                    <table class="w-full">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.code') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.disclosure') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.category') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.status') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.progress') }}</th>
                            </tr>
                        </thead>
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        @if($framework->is_mandatory)
                                            <span class="text-xs text-red-600 dark:text-red-400">{{ __('linscarbon.compliance.mandatory') }}</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        <select wire:change="updateCsrdStatus('{{ $framework->id }}', $event.target.value)"
                                            ...
                                            <option value="not_started" {{ $status === 'not_started' ? 'selected' : '' }}>{{ __('linscarbon.compliance.status.not_started') }}</option>
                                            <option value="in_progress" {{ $status === 'in_progress' ? 'selected' : '' }}>{{ __('linscarbon.compliance.status.in_progress') }}</option>
                                            <option value="compliant" {{ $status === 'compliant' ? 'selected' : '' }}>{{ __('linscarbon.compliance.status.compliant') }}</option>
                                            <option value="non_compliant" {{ $status === 'non_compliant' ? 'selected' : '' }}>{{ __('linscarbon.compliance.status.non_compliant') }}</option>
                                        </select>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-2">
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                        {{ __('linscarbon.compliance.no_frameworks') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($this->isoStandards as $standard)
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm">
                            ...
                            <div class="space-y-3">
                                <div>
                                    <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">{{ __('linscarbon.compliance.status') }}</label>
                                    <select wire:change="updateIsoStatus('{{ $standard->id }}', $event.target.value)"
                                        ...
                                        <option value="not_certified" {{ $status === 'not_certified' ? 'selected' : '' }}>{{ __('linscarbon.compliance.cert_status.not_certified') }}</option>
                                        <option value="in_progress" {{ $status === 'in_progress' ? 'selected' : '' }}>{{ __('linscarbon.compliance.cert_status.in_progress') }}</option>
                                        <option value="certified" {{ $status === 'certified' ? 'selected' : '' }}>{{ __('linscarbon.compliance.cert_status.certified') }}</option>
                                        <option value="expired" {{ $status === 'expired' ? 'selected' : '' }}>{{ __('linscarbon.compliance.cert_status.expired') }}</option>
                                    </select>
                                </div>

                                @if($certification && $certification->expiry_date)
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-gray-500 dark:text-gray-400">{{ __('linscarbon.compliance.expires') }}:</span>
                                ...
                                @if($certification && $certification->certifying_body)
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-gray-500 dark:text-gray-400">{{ __('linscarbon.compliance.certifier') }}:</span>
                    ...
                    @empty
                        <div class="col-span-full text-center py-8 text-gray-500 dark:text-gray-400">
                            {{ __('linscarbon.compliance.no_standards') }}
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                    <table class="w-full">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.task') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.type') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.priority') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.due_date') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.compliance.status') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.common.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            @if($task->status !== 'completed')
                                                <button wire:click="completeTask('{{ $task->id }}')"
                                                        ...
                                                        title="{{ __('linscarbon.compliance.mark_complete') }}">
                                            ...
                                            <button wire:click="openTaskModal('{{ $task->id }}')"
                                                    ...
                                                    title="{{ __('linscarbon.common.edit') }}">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                </svg>
                                            </button>
                                            <button wire:click="deleteTask('{{ $task->id }}')"
                                                    wire:confirm="{{ __('linscarbon.compliance.confirm_delete_task') }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                                        {{ __('linscarbon.compliance.no_tasks') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    ...
                    <form wire:submit="saveTask">
                        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                {{ $editingTaskId ? __('linscarbon.compliance.edit_task') : __('linscarbon.compliance.new_task') }}
                            </h3>
                        </div>

                        <div class="px-6 py-4 space-y-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('linscarbon.compliance.type') }}</label>
                                <select wire:model="taskForm.type" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                    ...
                                    <option value="internal">{{ __('linscarbon.compliance.internal') }}</option>
                                </select>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('linscarbon.compliance.title') }}</label>
                                <input type="text" wire:model="taskForm.title"
                                       class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                       placeholder="{{ __('linscarbon.compliance.task_title_placeholder') }}">
                                @error('taskForm.title') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('linscarbon.compliance.description') }}</label>
                                <textarea wire:model="taskForm.description" rows="3"
                                          class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                                          placeholder="{{ __('linscarbon.compliance.task_description_placeholder') }}"></textarea>
                            </div>

                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('linscarbon.compliance.priority') }}</label>
                                    <select wire:model="taskForm.priority" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                                        <option value="low">{{ __('linscarbon.compliance.priority.low') }}</option>
                                        <option value="medium">{{ __('linscarbon.compliance.priority.medium') }}</option>
                                        <option value="high">{{ __('linscarbon.compliance.priority.high') }}</option>
                                        <option value="critical">{{ __('linscarbon.compliance.priority.critical') }}</option>
                                    </select>
                                </div>

                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('linscarbon.compliance.due_date') }}</label>
                        ...
                        <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 flex justify-end space-x-3">
                            ...
                            <button type="submit"
                                ...
                                {{ $editingTaskId ? __('linscarbon.common.save') : __('linscarbon.common.create') }}
                            </button>
                        </div>
                    </form>
                </div>
