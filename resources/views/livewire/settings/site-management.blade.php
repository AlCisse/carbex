<div>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('linscarbon.settings.sites') }}</h1>
    ...
    <div class="space-y-4">
            ...
            <div class="flex items-start justify-between">
                <div class="flex items-start space-x-4">
                    ...
                    <div>
                        <div class="flex items-center space-x-2">
                            ...
                            @if ($site['is_primary'])
                                <x-badge variant="primary" size="sm">{{ __('linscarbon.sites.primary') }}</x-badge>
                            @endif
                            @if (!$site['is_active'])
                                <x-badge variant="warning" size="sm">{{ __('linscarbon.common.inactive') }}</x-badge>
                            @endif
                        </div>
                        ...
                        @if ($site['floor_area_m2'] || $site['employee_count'])
                            <div class="mt-2 flex space-x-4 text-sm text-gray-500">
                                ...
                                @if ($site['employee_count'])
                                    <span>{{ $site['employee_count'] }} {{ __('linscarbon.auth.employees') }}</span>
                                @endif
                            </div>
                        @endif
                    </div>
                ...
                <div class="flex items-center space-x-2">
                    @if (!$site['is_primary'])
                        ...
                        <button wire:click="setPrimary('{{ $site['id'] }}')" type="button" class="text-gray-400 hover:text-green-600" title="{{ __('linscarbon.sites.set_as_primary') }}">
                    ...
                    <button wire:click="openForm('{{ $site['id'] }}')" type="button" class="text-gray-400 hover:text-gray-600" title="{{ __('linscarbon.common.edit') }}">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    @if (!$site['is_primary'])
                    <button wire:click="confirmDelete('{{ $site['id'] }}')" type="button" class="text-gray-400 hover:text-red-600" title="{{ __('linscarbon.common.delete') }}">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <x-card class="text-center py-12">
            ...
            <h3 class="mt-2 text-sm font-semibold text-gray-900">{{ __('linscarbon.sites.no_sites') }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ __('linscarbon.sites.no_sites_desc') }}</p>
            @can('create', App\Models\Site::class)
            <div class="mt-6">
                <x-button wire:click="openForm" type="button">
                    <svg class="-ml-0.5 mr-1.5 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <form wire:submit="save">
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6">
                        <h3 class="text-lg font-medium leading-6 text-gray-900 mb-4">
                            {{ $editingSiteId ? __('linscarbon.sites.edit') : __('linscarbon.sites.add') }}
                        </h3>

                        <div class="space-y-4">
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <x-input wire:model="name" name="name" :label="__('linscarbon.sites.name')" required :error="$errors->first('name')" />
                                <x-input wire:model="code" name="code" :label="__('linscarbon.sites.code')" :hint="__('linscarbon.sites.code_hint')" :error="$errors->first('code')" />
                            </div>

                            <x-select wire:model="type" name="type" :label="__('linscarbon.sites.type')">
                                <option value="office">{{ __('linscarbon.sites.types.office') }}</option>
                                <option value="warehouse">{{ __('linscarbon.sites.types.warehouse') }}</option>
                                <option value="factory">{{ __('linscarbon.sites.types.factory') }}</option>
                                <option value="store">{{ __('linscarbon.sites.types.store') }}</option>
                                <option value="datacenter">{{ __('linscarbon.sites.types.datacenter') }}</option>
                                <option value="other">{{ __('linscarbon.sites.types.other') }}</option>
                            </x-select>

                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <x-input wire:model="address_line_1" name="address_line_1" :label="__('linscarbon.organization.address')" />
                                <x-input wire:model="city" name="city" :label="__('linscarbon.organization.city')" />
                            </div>

                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <x-input wire:model="postal_code" name="postal_code" :label="__('linscarbon.organization.postal_code')" />
                                <x-input wire:model="floor_area_m2" name="floor_area_m2" type="number" step="0.01" :label="__('linscarbon.sites.floor_area')" />
                            </div>

                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <x-input wire:model="employee_count" name="employee_count" type="number" :label="__('linscarbon.sites.employee_count')" />
                                <x-input wire:model="electricity_provider" name="electricity_provider" :label="__('linscarbon.sites.electricity_provider')" />
                            </div>
                            ...
                            <div class="flex items-center space-x-6">
                                <div class="flex items-center">
                                    ...
                                    <label for="renewable_energy" class="ml-2 text-sm text-gray-900">{{ __('linscarbon.sites.renewable_energy') }}</label>
                                </div>
                                @if (!$editingSiteId)
                                <div class="flex items-center">
                                    ...
                                    <label for="is_primary" class="ml-2 text-sm text-gray-900">{{ __('linscarbon.sites.is_primary') }}</label>
                                </div>
                                @endif
                            </div>

                            @if ($renewable_energy)
                            <x-input wire:model="renewable_percentage" name="renewable_percentage" type="number" step="0.1" min="0" max="100" :label="__('linscarbon.sites.renewable_percentage')" />
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                        <x-button type="submit" class="sm:ml-3">
                            {{ $editingSiteId ? __('linscarbon.common.save') : __('linscarbon.common.create') }}
                        </x-button>
                ...
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                            <h3 class="text-base font-semibold leading-6 text-gray-900">{{ __('linscarbon.sites.delete_title') }}</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">{{ __('linscarbon.sites.delete_confirm') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 px-4 pb-4 pt-5 sm:p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-medium leading-6 text-gray-900 dark:text-white">
                            {{ __('linscarbon.sites.import.title') }}
                        </h3>
                        <button wire:click="downloadTemplate" type="button" class="text-sm text-emerald-600 hover:text-emerald-700 flex items-center">
                            ...
                            {{ __('linscarbon.sites.import.download_template') }}
                        </button>
                    </div>
                    ...
                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            {{ __('linscarbon.sites.import.select_file') }}
                        </label>
                                ...
                                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                    ...
                                    @else
                                        <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                                            <span class="font-semibold">{{ __('linscarbon.sites.import.click_to_upload') }}</span>
                                        </p>
                    ...
                    @if(count($importErrors) > 0)
                        <div class="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 rounded-lg">
                            <h4 class="text-sm font-medium text-red-800 dark:text-red-400 mb-2">{{ __('linscarbon.sites.import.errors') }}</h4>
                    ...
                    @if(count($importPreview) > 0)
                        <div class="mb-4">
                            <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">
                                {{ __('linscarbon.sites.import.preview') }} ({{ count($importPreview) }} {{ __('linscarbon.sites.import.rows') }})
                            </h4>
                                ...
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0">
                                        <tr>
                                            ...
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.sites.name') }}</th>
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.sites.site_type') }}</th>
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.organization.city') }}</th>
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{{ __('linscarbon.sites.floor_area') }}</th>
                                        </tr>
                                    </thead>
                ...
                <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    @if(count($importPreview) > 0)
                        <x-button type="button" wire:click="confirmImport" class="sm:ml-3">
                            {{ __('linscarbon.sites.import.confirm') }} ({{ count($importPreview) }})
                        </x-button>
                    @endif
