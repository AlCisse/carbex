    ...
    @if($this->currentPlan)
        <div class="mb-6 rounded-lg bg-green-50 border border-green-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-sm font-medium text-green-800">{{ __('carbex.billing.current_plan') }}</h3>
                ...
                @if($this->currentPlan['is_trial'] && $this->currentPlan['trial_ends_at'])
                    <div class="text-right">
                        <p class="text-sm text-green-600">{{ __('carbex.billing.trial_ends') }}</p>
    ...
    <div class="flex justify-center mb-8">
        <div class="bg-gray-100 p-1 rounded-lg inline-flex">
            ...
            <button wire:click="setBillingPeriod('annual')"
                ...
                {{ __('carbex.billing.annual') }} <span class="text-green-600 text-xs ml-1">-17%</span>
            </button>
        </div>
    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border {{ $selectedPlan === $key ? 'border-green-500 ring-2 ring-green-500' : 'border-gray-200' }} p-6 relative flex flex-col">
                @if(isset($plan['popular']) && $plan['popular'])
                    <div class="absolute -top-3 left-1/2 transform -translate-x-1/2">
                        <span class="bg-green-500 text-white text-xs font-semibold px-3 py-1 rounded-full">{{ __('carbex.billing.popular') }}</span>
                    </div>
                @endif
                ...
                <div class="text-center flex-1">
                    ...
                    <div class="mt-4">
                        @if($key === 'free')
                            <span class="text-4xl font-bold text-gray-900">0{{ __('carbex.billing.currency') }}</span>
                            <span class="text-gray-500">/{{ $plan['duration'] }}</span>
                        @else
                            <span class="text-4xl font-bold text-gray-900">
                                {{ $billingPeriod === 'annual' ? $plan['price_annual'] : $plan['price_monthly'] }}{{ __('carbex.billing.currency') }}
                            </span>
                            <span class="text-gray-500">/{{ $billingPeriod === 'annual' ? __('carbex.billing.per_year') : __('carbex.billing.per_month') }}</span>
                        @endif
                    </div>

                    @if($billingPeriod === 'annual' && $key !== 'free')
                        <p class="mt-1 text-xs text-green-600">
                            {{ __('carbex.billing.save') }} {{ ($plan['price_monthly'] * 12) - $plan['price_annual'] }}{{ __('carbex.billing.currency') }}/{{ __('carbex.billing.year') }}
                        </p>
                    @endif
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <div class="flex justify-center gap-4 text-xs text-gray-500">
                            ...
                            <span>
                                ...
                                {{ $plan['limits']['sites'] ?? '∞' }} {{ __('carbex.billing.sites') }}
                            </span>
                        </div>
                    </div>
                ...
                <button wire:click="openPaymentModal('{{ $key }}')"
                    ...
                    @if($key === 'free')
                        {{ __('carbex.billing.start_trial') }}
                    @else
                        {{ __('carbex.billing.choose') }}
                    @endif
                </button>
            </div>
        @endforeach
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                    {{ __('carbex.billing.checkout_title') }}
                                </h3>
                        ...
                        <div class="mt-6 space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('carbex.billing.billing_period') }}</label>
                                <div class="grid grid-cols-2 gap-3">
                                    ...
                                    <button wire:click="setBillingPeriod('annual')"
                                        ...
                                        <span class="text-sm font-medium {{ $billingPeriod === 'annual' ? 'text-green-900' : 'text-gray-900' }}">
                                            {{ __('carbex.billing.annual') }}
                                        </span>
                                        <span class="text-xs text-green-600 mt-0.5">{{ __('carbex.billing.save') }} 17%</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <!-- Promo Code -->
                            <div>
                                <label for="promo-code" class="block text-sm font-medium text-gray-700">{{ __('carbex.billing.promo_code') }}</label>
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    <input type="text"
                                           ...
                                           placeholder="{{ __('carbex.billing.promo_placeholder') }}">
                                    <button type="button"
                            ...
                            <div class="bg-gray-50 rounded-lg p-4">
                                <div class="space-y-2">
                                    <div class="flex justify-between text-sm">
                                        <span class="text-gray-600">{{ $plans[$selectedPlan]['name'] }} ({{ $billingPeriod === 'annual' ? __('carbex.billing.annual') : __('carbex.billing.monthly') }})</span>
                                        <span class="text-gray-900">{{ $this->calculatedPrice['base'] }}{{ __('carbex.billing.currency') }}</span>
                                    </div>

                                    @if($this->calculatedPrice['discount'] > 0)
                                        <div class="flex justify-between text-sm">
                                            <span class="text-green-600">{{ __('carbex.billing.discount') }} (-{{ $this->calculatedPrice['discount_percent'] }}%)</span>
                                            <span class="text-green-600">-{{ number_format($this->calculatedPrice['discount'], 2) }}{{ __('carbex.billing.currency') }}</span>
                                        </div>
                                    @endif

                                    @if($this->calculatedPrice['annual_savings'] > 0)
                                        <div class="flex justify-between text-sm">
                                            <span class="text-green-600">{{ __('carbex.billing.annual_savings') }}</span>
                                            <span class="text-green-600">{{ $this->calculatedPrice['annual_savings'] }}{{ __('carbex.billing.currency') }}</span>
                                        </div>
                                    @endif

                                    <div class="pt-2 border-t border-gray-200">
                                        <div class="flex justify-between">
                                            <span class="text-base font-medium text-gray-900">{{ __('carbex.billing.total') }}</span>
                                            <span class="text-xl font-bold text-gray-900">{{ number_format($this->calculatedPrice['final'], 2) }}{{ __('carbex.billing.currency') }} <span class="text-sm font-normal text-gray-500">{{ $this->calculatedPrice['period_label'] }} HT</span></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center text-xs text-gray-500">
                                ...
                                {{ __('carbex.billing.secure_payment') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
                        <button wire:click="checkout"
                            ...
                            <span wire:loading.remove wire:target="checkout">
                                {{ __('carbex.billing.checkout_button') }}
                            </span>
