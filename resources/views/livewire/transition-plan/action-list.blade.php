<div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-lg font-semibold text-gray-900">{{ __('linscarbon.actions.title') }}</h2>
            <p class="mt-1 text-sm text-gray-500">{{ __('linscarbon.actions.subtitle') }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <div class="flex items-center space-x-4">
            <button wire:click="setFilter('all')" class="px-3 py-1.5 text-sm font-medium rounded-md {{ $filter === 'all' ? 'bg-green-100 text-green-700' : 'text-gray-600 hover:bg-gray-100' }}">
                {{ __('linscarbon.actions.filter_all') }} ({{ $statusCounts['all'] }})
            </button>
            <button wire:click="setFilter('todo')" class="px-3 py-1.5 text-sm font-medium rounded-md {{ $filter === 'todo' ? 'bg-yellow-100 text-yellow-700' : 'text-gray-600 hover:bg-gray-100' }}">
                {{ __('linscarbon.actions.status.todo') }} ({{ $statusCounts['todo'] }})
            </button>
            <button wire:click="setFilter('in_progress')" class="px-3 py-1.5 text-sm font-medium rounded-md {{ $filter === 'in_progress' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-100' }}">
                {{ __('linscarbon.actions.status.in_progress') }} ({{ $statusCounts['in_progress'] }})
            </button>
            <button wire:click="setFilter('completed')" class="px-3 py-1.5 text-sm font-medium rounded-md {{ $filter === 'completed' ? 'bg-green-100 text-green-700' : 'text-gray-600 hover:bg-gray-100' }}">
                {{ __('linscarbon.actions.status.completed') }} ({{ $statusCounts['completed'] }})
            </button>
        </div>
    </div>
                    ...
                    <li class="p-4 hover:bg-gray-50">
                        <div class="flex items-start justify-between">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center space-x-3">
                                    ...
                                    @if($action->isCompleted())
                                        <button wire:click="updateStatus('{{ $action->id }}', 'in_progress')" class="flex-shrink-0 h-5 w-5 rounded-full bg-green-500 flex items-center justify-center" title="{{ __('linscarbon.actions.reopen') }}">
                                            <svg class="h-3 w-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                                            </svg>
                                        </button>
                                    @elseif($action->isInProgress())
                                        <button wire:click="updateStatus('{{ $action->id }}', 'completed')" class="flex-shrink-0 h-5 w-5 rounded-full border-2 border-blue-500 bg-blue-100" title="{{ __('linscarbon.actions.complete') }}"></button>
                                    @else
                                        <button wire:click="updateStatus('{{ $action->id }}', 'in_progress')" class="flex-shrink-0 h-5 w-5 rounded-full border-2 border-gray-300" title="{{ __('linscarbon.actions.start') }}"></button>
                                    @endif
                                    ...
                                    @if($action->isTodo())
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 text-yellow-800">
                                            {{ __('linscarbon.actions.status.todo') }}
                                        </span>
                                    @elseif($action->isInProgress())
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ __('linscarbon.actions.status.in_progress') }}
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">
                                            {{ __('linscarbon.actions.status.completed') }}
                                        </span>
                                    @endif
                                    ...
                                    <span class="flex items-center">
                                        @if($action->difficulty === 'easy')
                                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-50 text-green-700">
                                                {{ __('linscarbon.actions.difficulty.easy') }}
                                            </span>
                            ...
                            <div class="flex items-center space-x-2 ml-4">
                                <button wire:click="openEditModal('{{ $action->id }}')" class="text-indigo-600 hover:text-indigo-900" title="{{ __('linscarbon.common.edit') }}">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button wire:click="deleteAction('{{ $action->id }}')" wire:confirm="{{ __('linscarbon.actions.confirm_delete') }}" class="text-red-600 hover:text-red-900" title="{{ __('linscarbon.common.delete') }}">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @else
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('linscarbon.actions.empty') }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ __('linscarbon.actions.empty_description') }}</p>
            <div class="mt-6">
                <button wire:click="openCreateModal" type="button" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
                    ...
                    {{ __('linscarbon.actions.create_first') }}
                </button>
            </div>
        </div>
    @endif
                ...
                <form wire:submit="save">
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                            {{ $editingId ? __('linscarbon.actions.edit') : __('linscarbon.actions.new') }}
                        </h3>
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div>
                                <label for="title" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.actions.form.title') }} <span class="text-red-500">*</span>
                                </label>
                                <input wire:model="title" type="text" id="title" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm" placeholder="{{ __('linscarbon.actions.form.title_placeholder') }}">
                            ...
                            <div>
                                <label for="description" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.actions.form.description') }}
                                </label>
                                <textarea wire:model="description" id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm" placeholder="{{ __('linscarbon.actions.form.description_placeholder') }}"></textarea>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label for="dueDate" class="block text-sm font-medium text-gray-700">
                                        {{ __('linscarbon.actions.form.due_date') }}
                                    </label>
                                ...
                                <div>
                                    <label for="categoryId" class="block text-sm font-medium text-gray-700">
                                        {{ __('linscarbon.actions.form.category') }}
                                    </label>
                                    <select wire:model="categoryId" id="categoryId" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                        <option value="">{{ __('linscarbon.actions.form.category_none') }}</option>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label for="status" class="block text-sm font-medium text-gray-700">
                                        {{ __('linscarbon.actions.form.status') }}
                                    </label>
                                    <select wire:model="status" id="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm">
                                        <option value="todo">{{ __('linscarbon.actions.status.todo') }}</option>
                                        <option value="in_progress">{{ __('linscarbon.actions.status.in_progress') }}</option>
                                        <option value="completed">{{ __('linscarbon.actions.status.completed') }}</option>
                                    </select>
                                ...
                                <div>
                                    <label for="estimatedCost" class="block text-sm font-medium text-gray-700">
                                        {{ __('linscarbon.actions.form.estimated_cost') }}
                                    </label>
                            ...
                            <div>
                                <label for="co2ReductionPercent" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.actions.form.co2_reduction') }} <span class="text-green-600 font-semibold">{{ $co2ReductionPercent }}%</span>
                                </label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">
                                    {{ __('linscarbon.actions.form.difficulty') }}
                                </label>
                                <div class="flex space-x-4">
                                    <label class="flex items-center">
                                        ...
                                        <span class="ml-2 text-sm text-gray-700">{{ __('linscarbon.actions.difficulty.easy') }}</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input wire:model="difficulty" type="radio" value="medium" class="h-4 w-4 text-yellow-600 focus:ring-yellow-500 border-gray-300">
                                        <span class="ml-2 text-sm text-gray-700">{{ __('linscarbon.actions.difficulty.medium') }}</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input wire:model="difficulty" type="radio" value="hard" class="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300">
                                        <span class="ml-2 text-sm text-gray-700">{{ __('linscarbon.actions.difficulty.hard') }}</span>
                                    </label>
                                </div>
