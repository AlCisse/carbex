<div>
    ...
    <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
        <div class="flex">
            ...
            <div class="ml-3">
                <h3 class="text-sm font-medium text-blue-800">{{ __('linscarbon.targets.sbti_title') }}</h3>
                <div class="mt-2 text-sm text-blue-700">
                    <p>{{ __('linscarbon.targets.sbti_description') }}</p>
                    <ul class="mt-2 list-disc list-inside space-y-1">
                        <li><strong>{{ __('linscarbon.targets.sbti_scope12_rate') }}</strong> {{ __('linscarbon.targets.sbti_scope12_label') }}</li>
                        <li><strong>{{ __('linscarbon.targets.sbti_scope3_rate') }}</strong> {{ __('linscarbon.targets.sbti_scope3_label') }}</li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h2 class="text-lg font-semibold text-gray-900">{{ __('linscarbon.targets.title') }}</h2>
            <p class="mt-1 text-sm text-gray-500">{{ __('linscarbon.targets.subtitle') }}</p>
        </div>
                    ...
                    <div class="p-6">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    @if($target->is_sbti_aligned)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                            ...
                                            {{ __('linscarbon.targets.sbti_aligned') }}
                                        </span>
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                            {{ __('linscarbon.targets.sbti_not_aligned') }}
                                        </span>
                                    @endif
                                </div>
                                <p class="mt-1 text-sm text-gray-500">
                                    {{ __('linscarbon.targets.horizon', ['years' => $target->years_to_target]) }}
                                </p>
                            </div>
                            <div class="flex items-center space-x-2">
                                <button wire:click="openEditModal('{{ $target->id }}')" class="text-indigo-600 hover:text-indigo-900" title="{{ __('linscarbon.common.edit') }}">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button wire:click="deleteTarget('{{ $target->id }}')" wire:confirm="{{ __('linscarbon.targets.confirm_delete') }}" class="text-red-600 hover:text-red-900" title="{{ __('linscarbon.common.delete') }}">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="mt-6 grid grid-cols-3 gap-6">
                            ...
                            <div class="bg-orange-50 rounded-lg p-4">
                                ...
                                <p class="mt-1 text-xs text-orange-700">{{ $target->scope_1_annual_rate }}%/{{ __('linscarbon.targets.per_year') }}</p>
                            </div>
                            ...
                            <div class="bg-yellow-50 rounded-lg p-4">
                                ...
                                <p class="mt-1 text-xs text-yellow-700">{{ $target->scope_2_annual_rate }}%/{{ __('linscarbon.targets.per_year') }}</p>
                            </div>
                            ...
                            <div class="bg-blue-50 rounded-lg p-4">
                                ...
                                <p class="mt-1 text-xs text-blue-700">{{ $target->scope_3_annual_rate }}%/{{ __('linscarbon.targets.per_year') }}</p>
                            </div>
                        </div>
    ...
    @else
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <h3 class="mt-2 text-sm font-medium text-gray-900">{{ __('linscarbon.targets.empty') }}</h3>
            <p class="mt-1 text-sm text-gray-500">{{ __('linscarbon.targets.empty_description') }}</p>
            <div class="mt-6">
                <button wire:click="openCreateModal" type="button" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700">
                    ...
                    {{ __('linscarbon.targets.create_first') }}
                </button>
            </div>
        </div>
    @endif
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="relative z-50 inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl sm:w-full sm:p-6">
                <form wire:submit="save">
                    <div>
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                            {{ $editingId ? __('linscarbon.targets.edit') : __('linscarbon.targets.new') }}
                        </h3>
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label for="baselineYear" class="block text-sm font-medium text-gray-700">
                                        {{ __('linscarbon.targets.baseline_year') }} <span class="text-red-500">*</span>
                                    </label>
                                ...
                                <div>
                                    <label for="targetYear" class="block text-sm font-medium text-gray-700">
                                        {{ __('linscarbon.targets.target_year') }} <span class="text-red-500">*</span>
                                    </label>
                            ...
                            <div class="flex justify-end">
                                <button type="button" wire:click="applySbtiDefaults" class="inline-flex items-center px-3 py-1.5 border border-blue-300 rounded-md text-sm font-medium text-blue-700 bg-blue-50 hover:bg-blue-100">
                                    ...
                                    {{ __('linscarbon.targets.apply_sbti') }}
                                </button>
                            </div>
                            ...
                            <div>
                                <label for="scope1Reduction" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.targets.scope1_reduction') }}
                                    <span class="text-orange-600 font-semibold">{{ $scope1Reduction }}%</span>
                                    <span class="text-xs text-gray-500">({{ $sbtiCompliance['scope1_annual'] ?? 0 }}%/{{ __('linscarbon.targets.per_year') }})</span>
                            ...
                            <div>
                                <label for="scope2Reduction" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.targets.scope2_reduction') }}
                                    <span class="text-yellow-600 font-semibold">{{ $scope2Reduction }}%</span>
                                    <span class="text-xs text-gray-500">({{ $sbtiCompliance['scope2_annual'] ?? 0 }}%/{{ __('linscarbon.targets.per_year') }})</span>
                            ...
                            <div>
                                <label for="scope3Reduction" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.targets.scope3_reduction') }}
                                    <span class="text-blue-600 font-semibold">{{ $scope3Reduction }}%</span>
                                    <span class="text-xs text-gray-500">({{ $sbtiCompliance['scope3_annual'] ?? 0 }}%/{{ __('linscarbon.targets.per_year') }})</span>
                            ...
                            <div>
                                <label for="notes" class="block text-sm font-medium text-gray-700">
                                    {{ __('linscarbon.targets.notes') }}
                                </label>
                                <textarea wire:model="notes" id="notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 sm:text-sm" placeholder="{{ __('linscarbon.targets.notes_placeholder') }}"></textarea>
