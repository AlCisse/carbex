<div class="max-w-4xl mx-auto">
    ...
    <nav aria-label="Progress" class="mb-8">
        <ol class="flex items-center justify-center space-x-5">
            @foreach([1 => __('linscarbon.banking_wizard.country'), 2 => __('linscarbon.banking_wizard.bank'), 3 => __('linscarbon.banking_wizard.authorize'), 4 => __('linscarbon.banking_wizard.connect'), 5 => __('linscarbon.banking_wizard.done')] as $stepNum => $stepName)
    ...
    @if($step === 1)
        <x-card>
            <x-slot name="header">
                <h3 class="text-lg font-semibold">{{ __('linscarbon.banking_wizard.select_country') }}</h3>
                <p class="text-sm text-gray-500">{{ __('linscarbon.banking_wizard.supported_countries') }}</p>
            </x-slot>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button wire:click="selectCountry('FR')"
                    ...
                    <div class="text-left">
                        <span class="block font-semibold">{{ __('linscarbon.emission_factors.countries.fr') }}</span>
                        <span class="text-sm text-gray-500">{{ __('linscarbon.banking_wizard.via') }} Bridge.io</span>
                    </div>
                </button>
                ...
                <button wire:click="selectCountry('DE')"
                    ...
                    <span class="text-4xl mr-4">🇩🇪</span>
                    <div class="text-left">
                        <span class="block font-semibold">{{ __('linscarbon.emission_factors.countries.de') }}</span>
                        <span class="text-sm text-gray-500">{{ __('linscarbon.banking_wizard.via') }} Finapi</span>
                    </div>
                </button>
            </div>
        </x-card>
        ...
        <x-card>
            <x-slot name="header">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold">{{ __('linscarbon.banking_wizard.select_bank') }}</h3>
                        <p class="text-sm text-gray-500">{{ __('linscarbon.banking_wizard.supported_banks') }}</p>
                    </div>
            ...
            <div class="mb-6">
                <x-input
                    ...
                    type="search"
                    placeholder="{{ __('linscarbon.banking_wizard.search_banks') }}"
                    class="w-full"
                />
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-96 overflow-y-auto">
                ...
                @empty
                    <div class="col-span-full text-center py-8 text-gray-500">
                        {{ __('linscarbon.banking_wizard.no_banks_found') }}
                    </div>
                @endforelse
            </div>
        </x-card>
    ...
    @if($step === 3)
        <x-card>
            <x-slot name="header">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-lg font-semibold">{{ __('linscarbon.banking_wizard.authorize_connection') }}</h3>
                        <p class="text-sm text-gray-500">{{ __('linscarbon.banking_wizard.redirect_to_bank') }}</p>
                    </div>
                    <button wire:click="previousStep" class="text-sm text-gray-500 hover:text-gray-700">
                        &larr; {{ __('linscarbon.back') }}
                    </button>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    <div class="flex">
                        ...
                        <div>
                            <h4 class="font-semibold text-blue-800 dark:text-blue-200">{{ __('linscarbon.banking_wizard.secure_connection') }}</h4>
                            <p class="text-sm text-blue-700 dark:text-blue-300 mt-1">
                                {{ __('linscarbon.banking_wizard.psd2_info') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                {{-- What we access --}}
                <div>
                    <h4 class="font-semibold mb-3">{{ __('linscarbon.banking_wizard.what_we_access') }}</h4>
                    <ul class="space-y-2">
                        <li class="flex items-center text-sm">
                            <x-heroicon-s-check-circle class="w-5 h-5 text-green-500 mr-2" />
                            {{ __('linscarbon.banking_wizard.account_balances') }}
                        </li>
                        <li class="flex items-center text-sm">
                            <x-heroicon-s-check-circle class="w-5 h-5 text-green-500 mr-2" />
                            {{ __('linscarbon.banking_wizard.transaction_history') }}
                        </li>
                        <li class="flex items-center text-sm">
                            <x-heroicon-s-check-circle class="w-5 h-5 text-green-500 mr-2" />
                            {{ __('linscarbon.banking_wizard.transaction_details') }}
                        </li>
                    </ul>
                </div>
                ...
                <div>
                    <h4 class="font-semibold mb-3">{{ __('linscarbon.banking_wizard.what_we_dont_access') }}</h4>
                    <ul class="space-y-2">
                        <li class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                            <x-heroicon-s-x-circle class="w-5 h-5 text-red-500 mr-2" />
                            {{ __('linscarbon.banking_wizard.login_credentials') }}
                        </li>
                        <li class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                            <x-heroicon-s-x-circle class="w-5 h-5 text-red-500 mr-2" />
                            {{ __('linscarbon.banking_wizard.ability_transfers') }}
                        </li>
                        <li class="flex items-center text-sm text-gray-600 dark:text-gray-400">
                            <x-heroicon-s-x-circle class="w-5 h-5 text-red-500 mr-2" />
                            {{ __('linscarbon.banking_wizard.investment_details') }}
                        </li>
                    </ul>
                </div>
                ...
                <div class="flex justify-end space-x-3 pt-4 border-t dark:border-gray-700">
                    ...
                    <x-button wire:click="initiateConnection" wire:loading.attr="disabled">
                        <span wire:loading.remove wire:target="initiateConnection">
                            {{ __('linscarbon.banking_wizard.continue_to_bank') }}
                        </span>
                        <span wire:loading wire:target="initiateConnection">
                            {{ __('linscarbon.banking_wizard.redirecting') }}
                        </span>
                    </x-button>
                </div>
            </div>
    ...
    @if($step === 4 && $redirectUrl)
        <x-card class="text-center py-12">
            <div class="animate-pulse">
                ...
                <h3 class="text-lg font-semibold mb-2">{{ __('linscarbon.banking_wizard.redirecting_to_bank') }}</h3>
                <p class="text-gray-500 mb-6">{{ __('linscarbon.banking_wizard.complete_authorization') }}</p>
                <a href="{{ $redirectUrl }}" target="_blank" class="text-green-600 hover:underline">
                    {{ __('linscarbon.banking_wizard.click_if_not_redirected') }}
                </a>
            </div>
        </x-card>
    ...
    @if($step === 5 && $connectionSuccess)
        <x-card class="text-center py-12">
            ...
            <h3 class="text-2xl font-bold mb-2">{{ __('linscarbon.banking_wizard.bank_connected') }}</h3>
            <p class="text-gray-500 mb-6">{{ __('linscarbon.banking_wizard.sync_in_progress') }}</p>

            <div class="flex justify-center space-x-4">
                <x-button wire:click="resetWizard" variant="secondary">
                    {{ __('linscarbon.banking_wizard.connect_another') }}
                </x-button>
    ...
    @if($this->hasActiveConnections && $step === 1)
        <div class="mt-8">
            <h3 class="text-lg font-semibold mb-4">{{ __('linscarbon.banking_wizard.connected_banks') }}</h3>
            <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <div class="flex items-center text-sm text-gray-500">
                                    <span class="mr-2">{{ $connection->accounts->count() }} {{ __('linscarbon.banking_wizard.accounts') }}</span>
                                    @if($connection->last_sync_at)
                                        <span>{{ __('linscarbon.banking_wizard.synced') }} {{ $connection->last_sync_at->diffForHumans() }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            ...
                            <button wire:click="syncConnection('{{ $connection->id }}')"
                                class="p-2 text-gray-500 hover:text-green-600" title="{{ __('linscarbon.banking_wizard.sync_now') }}">
                                <x-heroicon-o-arrow-path class="w-5 h-5" />
                            </button>
                            <button wire:click="disconnectBank('{{ $connection->id }}')"
                                wire:confirm="{{ __('linscarbon.banking_wizard.disconnect_confirm') }}"
                                class="p-2 text-gray-500 hover:text-red-600" title="{{ __('linscarbon.banking_wizard.disconnect') }}">
