        ...
        <div x-show="showImport" x-cloak class="fixed inset-0 z-50 overflow-y-auto" dusk="import-modal">
            <div class="flex items-center justify-center min-h-screen p-4">
                ...
                <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
                    <h3 class="text-lg font-semibold mb-4">{{ __('carbex.emissions.import_title') }}</h3>
                    <p class="text-sm text-gray-500 mb-4">{{ __('carbex.emissions.import_description') }}</p>
                    <div class="flex justify-end gap-3">
                        <button @click="showImport = false" class="px-4 py-2 border border-gray-300 rounded-md text-sm">{{ __('carbex.cancel') }}</button>
                        <button class="px-4 py-2 bg-green-600 text-white rounded-md text-sm">{{ __('carbex.emissions.import') }}</button>
                    </div>
                </div>
            </div>
        </div>
