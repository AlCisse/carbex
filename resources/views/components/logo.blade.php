...
<div {{ $attributes->merge(['class' => 'flex items-center space-x-2']) }}>
    @if($variant === 'premium')
        ...
        <div class="{{ $s['container'] }} rounded-lg flex items-center justify-center" style="background: linear-gradient(135deg, var(--accent) 0%, #14b8a6 100%);">
    ...
    @else
        {{-- Default variant --}}
        <div class="{{ $s['container'] }} rounded-lg flex items-center justify-center" style="background-color: var(--accent);">
            <svg class="{{ $s['icon'] }} text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($showText)
        <span class="{{ $s['text'] }} font-semibold {{ $textClass }}" style="color: var(--text-primary);">Carbex</span>
    @endif
</div>
